    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-gray-500/20 backdrop-blur-md shadow-sm border-b border-pink-300">
      <div className="flex flex-wrap items-center justify-between mx-auto px-4 py-2">
        ...
        <div className="block md:hidden mobile-menu">
         {
          !navbarOpen ? (
            <button onClick={() => setNavbarOpen(true)} className="text-slate-200 flex items-center px-3 py-2 border border-slate-200 hover:text-white hover:border-white">
               <Bars3Icon className="h-5 w-5" />
            </button>
          ) : (
            <button onClick={() => setNavbarOpen(false)} className="text-slate-200 flex items-center px-3 py-2 border border-slate-200 hover:text-white hover:border-white">
